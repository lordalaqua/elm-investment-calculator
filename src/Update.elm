...
update msg model =
    case msg of
        StartValue start_value ->
          { model | start_value = Result.withDefault 0 (String.toFloat start_value) }

        Deposit deposit ->
          { model | deposit = Result.withDefault 0 (String.toFloat deposit) }

        Rate rate ->
          { model | rate = Result.withDefault 0 (String.toFloat rate) }
